...
# integraded AUC for models that have "lp" (cph/gbdt)
scores_auc <- bmr$score(msr("surv.uno_auc"))
...
scores_ibs$score_name <- "IBS (lower is better)"
colnames(scores_auc)[ncol(scores_auc)] <- "score"
scores_auc$score_name <- "IAUC (higher is better)"


scores <- rbind(scores_cix, scores_ibs, scores_auc)
...
scores$score_name <- factor(scores$score_name,
                            levels=c("C-index (higher is better)", "IBS (lower is better)", "IAUC (higher is better)"))

library(ggplot2)
library(ggstatsplot)
library(patchwork)

s_comparison <- scores %>% filter(
    task_id == "risk + annot + pyrad",
    learner_id != "DeepHit", # it does not converge,
    score > 0.5 | score < 0.22
  )

s_comparison %>% group_by(learner_id, score_name) %>% summarise(score_m=mean(score), score_sd=sd(score))
...
p_comparison <- grouped_ggbetweenstats(
  ...
  centrality.label.args = list(size=2.5),
  plotgrid.args = list(widths=c(6, 6, 3))
) & theme(axis.text=element_text(size=8.75))

p_comparison

ggsave(p_comparison, filename = "../results/model_comparison.pdf", width = 10.5, height = 5)


s_bb <-  scores %>% filter(learner_id == "GBDT")
s_bb %>% group_by(task_id, score_name) %>% summarise(score_m=mean(score), score_sd=sd(score))
...
ggsave(p_bb, filename = "../results/bb_performance.pdf", width = 10.5, height = 5)


s_wb <- scores %>% filter(learner_id == "CoxPH")
s_wb %>% group_by(task_id, score_name) %>% summarise(score_m=mean(score), score_sd=sd(score))
...
ggsave(p_wb, filename = "../results/wb_performance.pdf", width = 10.5, height = 5)

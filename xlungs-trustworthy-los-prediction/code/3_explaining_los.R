...
df_plot_vi_long$name = factor(
  df_plot_vi_long$name, 
  levels = unique(df_plot_vi_long$name)[c(4,2,1,6,3,5)]
)
...
ggsave(p_exp1, filename = "../results/global_explanations.pdf", width = 11, height = 5)
...
df_plot_survshap_long$name = factor(
  df_plot_survshap_long$name, 
  levels = unique(df_plot_survshap_long$name)[c(3,5,2,1,6,4)]
)
...
p_tsv <- ggplot(data = df_plot_survshap_long, 
  ...
  scale_color_manual(NULL, values = DALEX::colors_discrete_drwhy(6)[c(1, 4, 2, 3, 5, 6)]) +
  DALEX::theme_drwhy()
p_tsv
...
ggsave(p_exp2, filename = "../results/local_explanations.pdf", width = 11, height = 5)
